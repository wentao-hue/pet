...
| Demote cold file pages by inode `open_count` | inode counter, cold-file processing generation, reopen-safe migration, runtime disable drains queued inode refs; `generic_shutdown_super()` drains per-superblock queue/processing entries so umount never sees PET-held inode references (references stay visible via list or detached count until after `iput()`); already-placed folios are skipped and dirty/writeback-skipped inodes are requeued for a post-writeback retry | done |
...
| Prerequisite: NUMA balancing built in, off at runtime | `CONFIG_NUMA_BALANCING` is a Kconfig dependency: without it `pte_protnone()` is stubbed to 0 and canary faults would loop forever. At runtime PET reuses the PROT_NONE encoding of NUMA hint faults; with `kernel.numa_balancing=1`, balancing-poisoned PTEs at canary addresses are miscounted as fake faults and every hint fault pays the PET fault-path cost. Enabling PET while balancing is on logs a warning | enforced + documented |
